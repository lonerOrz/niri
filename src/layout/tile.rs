        ...
        Point::from((0., y))
            ...
            fn render_inner<'a, R: NiriRenderer + 'a>(
                ...
                let blur_config = self
                    ...
                    .rules()
                    .blur
                    .resolve_against(self.options.layout.blur);
                ...
                // Re-enable blur in overview mode, but note that this implementation has known bugs.
                // The blur rendering in overview mode may have issues where blur only appears in the
                // top-left corner of windows and doesn't follow window movements correctly.
                // TODO: Consider conditionally rendering blur based on the workspace background
                // transparency and other factors specific to overview mode. For example, different
                // approaches may be needed for rendering blur effects depending on whether the
                // workspace background is transparent or opaque.
                let blur_element = (blur_config.on && output.is_some())
                    .then(|| {
                        // If the window is floating, don't use optimized blur.
